# -*- coding: utf-8 -*-
"""
Joke Video Engine v4
Advanced Video Generation Engine for Joke Videos.
Features:
- Object-Oriented Design
- Advanced random Ken Burns effects (Zoom In, Zoom Out, Pan)
- Sound Effect injection for punchlines
- Better subtitle rendering logic
"""
...
class JokeVideoEngine:
    def __init__(self, script_path, output_path="joke_video_v4.mp4", assets_dir="assets"):
    ...
    async def _generate_voice(self, text, filename, character="default"):
    ...
    def _apply_random_ken_burns(self, clip, duration):
        ...
        if effect_type == 'zoom_in':
            ...
            # Simulated by zooming in slightly, then moving center
            # Requires custom positioning, simplified here to just slight zoom for stability
            return clip.with_effects([Resize(lambda t: 1.05 + 0.05 * (t / duration))])
        else:
            return clip.with_effects([Resize(lambda t: 1.05 + 0.05 * (t / duration))])
    ...
    def _add_subtitle_to_image(self, img_path, text, output_path):
        ...
        try:
            font_sub = ImageFont.truetype(self.font_path, 60)
        except:
        ...
        draw.rectangle([0, 1080-bar_h, 1920, 1080], fill=(0, 0, 0, 200)) # Darker background
        ...
        for line in wrapped_lines:
            ...
            draw.text((x_pos+2, y_start+2), line, font=font_sub, fill=(0, 0, 0))
            draw.text((x_pos, y_start), line, font=font_sub, fill=(255, 255, 0)) # Yellow text for better contrast
            y_start += 70
            
        base_img.save(output_path)

    def generate(self):
        print(f"Starting Video Generation V4 for {self.script_path}")
        ...
        for i, scene in enumerate(scenes):
            ...
            if scene["voice"]:
                asyncio.run(self._generate_voice(scene["voice"], audio_file, scene["character"]))
            ...
            # Apply dynamic Ken Burns
            img_clip = self._apply_random_ken_burns(img_clip, audio_clip.duration)
        ...
        if os.path.exists(bgm_path):
            ...
            bgm = AudioFileClip(bgm_path).with_effects([MultiplyVolume(0.10)])
...
if __name__ == "__main__":
    ...
    output = sys.argv[2] if len(sys.argv) > 2 else "bragging_dog_v4.mp4"
