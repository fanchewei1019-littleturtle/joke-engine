# -*- coding: utf-8 -*-
"""
Joke Video Engine v14 (SiliconFlow Flux Edition) 🐢🎥✨
New Features:
- Integrated SiliconFlow Flux.1-schnell API for High-Quality Image Generation
- Automatic Image Prompt Extraction from Script
- Improved Asset Management & Fallback Logic
...
class JokeVideoEngineV14:
    def __init__(self, script_path, output_path="joke_v14.mp4", is_shorts=False, assets_dir="assets_v14"):
        ...
        self.avatar_files = {
            "狗狗": os.path.join("assets", "avatars", "avatar_cute.jpg"),
            "屋主": os.path.join("assets", "avatars", "avatar.jpg"),
            "default": os.path.join("assets", "avatars", "avatar_cute.jpg")
        }
    ...
    def _add_bouncing_avatar(self, character, start_t, duration, is_punchline=False):
        ...
        if character == "狗狗": base_x = 80 if self.is_shorts else 150
        elif character == "屋主": base_x = self.size[0] - 330 if self.is_shorts else self.size[0] - 400
        else: base_x = self.size[0] // 2 - 125
        def bounce(t):
            y_offset = -20 * abs(np.sin(2 * np.pi * 2.0 * t))
            if is_punchline: y_offset += random.randint(-8, 8)
    ...
    def _generate_images_siliconflow(self, scenes):
        print("--- SiliconFlow Image Generation Started ---")
        if SILICONFLOW_API_KEY == "sk-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx":
            print("[Warning] SiliconFlow API Key is not set. Skipping generation.")
            return
        ...
        for i, scene in enumerate(scenes):
            ...
            payload = {
                ...
                "prompt": f"{prompt}, high quality, cinematic, joke video style",
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"Failed to generate image. Status: {response.status_code}, Error: {response.text}")
            except Exception as e:
                print(f"Error during SiliconFlow generation: {e}")

    def generate(self):
        print(f" V14 Engine (SiliconFlow Flux) Started - Shorts: {self.is_shorts}", flush=True)
        scenes, video_title = self._parse_script()
        
        # Step 1: Generate Images
        self._generate_images_siliconflow(scenes)
        ...
        for i, scene in enumerate(scenes):
            ...
            for d_idx, dial in enumerate(scene["dialogues"]):
                ...
                rate, pitch, _ = self._analyze_emotion_and_emoji(text, scene["is_punchline"], char)
        ...
        print(" V14 SiliconFlow Edition Complete!", flush=True)
...
if __name__ == "__main__":
    ...
    output = sys.argv[2] if len(sys.argv) > 2 else "joke_v14.mp4"
    assets = sys.argv[3] if len(sys.argv) > 3 else "assets_v14"
    engine = JokeVideoEngineV14(script, output, is_shorts=True, assets_dir=assets)
    engine.generate()
