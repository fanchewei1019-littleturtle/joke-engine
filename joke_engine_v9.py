# -*- coding: utf-8 -*-
"""
Joke Video Engine v9
Next-Generation Video Generation Engine for Joke Videos.
New Features in v9:
- Character-Specific Subtitle Colors! (Dog: Blue, Owner: Green, Narrator: Yellow, Punchline: Red)
- Punchline Audio Echo/Reverb Effect!
- Automatic YouTube Thumbnail Generation!
- Glitch/Flash visual effects for transitions and punchlines
"""
...
class JokeVideoEngineV9:
    def __init__(self, script_path, output_path="joke_video_v9.mp4", assets_dir="assets"):
    ...
    def _apply_vfx(self, clip, duration, character, is_punchline):
        # Ken Burns
        if is_punchline:
            clip = clip.with_effects([Resize(lambda t: 1.0 + 0.5 * (t / duration))])
            # Screen Shake for the first 0.6 seconds of punchline
            shake_func = lambda t: ('center', 'center') if t > 0.6 else (
                1920/2 + random.randint(-20, 20), 
                1080/2 + random.randint(-20, 20)
            )
            ...
            # Flash effect
            flash = ColorClip(clip.size, color=(255, 255, 255)).with_duration(0.3)
            clip = CompositeVideoClip([clip, flash.with_position("center").with_effects([CrossFadeOut(duration=0.3)])])

        elif character == 'dog':
            clip = clip.with_effects([Resize(lambda t: 1.0 + 0.18 * (t / duration))])
        elif character == 'owner':
            clip = clip.with_effects([Resize(lambda t: 1.18)])
        else:
            clip = clip.with_effects([Resize(lambda t: 1.18 - 0.18 * (t / duration))])

        return clip

    def _add_subtitle_to_image(self, img_path, text, output_path, emoji="", character="default", is_punchline=False):
        ...
        # Determine subtitle color based on character and punchline
        fill_color = (255, 240, 80, 255) # Yellow default
        if is_punchline:
            fill_color = (255, 80, 80, 255) # Red for punchline
        elif character == "dog":
            fill_color = (173, 216, 230, 255) # Light Blue
        elif character == "owner":
            fill_color = (144, 238, 144, 255) # Light Green
        ...
        for line in wrapped_lines:
            ...
            # Inner Text with Character Color
            draw.text((x_pos, y_start), line, font=font_sub, fill=fill_color) 
            y_start += 100
    ...
    def _create_intro_slide(self, title):
        img = Image.new('RGB', (1920, 1080), color=(15, 15, 25)) # Darker background for V9
        ...
        draw.text((x, y), title, font=fnt_large, fill=(255, 150, 0)) # Orange text for V9
        ...
        y2 = 620
        draw.text((x2, y2), sub, font=fnt_small, fill=(200, 255, 200))
        
        intro_path = "temp_intro.jpg"
        img.save(intro_path)
        
        # 2.5 seconds intro with a slight zoom
        clip = ImageClip(intro_path).with_duration(2.5).with_effects([Resize(lambda t: 1.0 + 0.08 * (t / 2.5))])
        return clip

    def _create_thumbnail(self, title):
        img = Image.new('RGB', (1280, 720), color=(255, 80, 80)) # Vibrant Red
        draw = ImageDraw.Draw(img)
        try:
            fnt_large = ImageFont.truetype(self.font_path, 120)
        ...
        draw.rectangle([50, 50, 1230, 670], outline=(255, 255, 255), width=15)
        
        try:
            bbox = draw.textbbox((0, 0), "🔥 " + title + " 🔥", font=fnt_large)
            x = (1280 - (bbox[2] - bbox[0])) / 2
        except:
            x = (1280 - len("🔥 " + title + " 🔥")*120) / 2
            
        draw.text((x+5, 280+5), "🔥 " + title + " 🔥", font=fnt_large, fill=(0, 0, 0))
        draw.text((x, 280), "🔥 " + title + " 🔥", font=fnt_large, fill=(255, 255, 0))
        
        thumb_path = self.output_path.rsplit(".", 1)[0] + "_thumbnail.jpg"
        img.save(thumb_path)
        print(f"Thumbnail saved to {thumb_path}")

    def generate(self):
        print(f"Starting Video Generation V9 for {self.script_path}")
        ...
        for i, scene in enumerate(scenes):
            ...
            if scene["voice"]:
                ...
                # Punchline Echo Effect in V9!
                if scene["is_punchline"]:
                    echo_clip = audio_clip.with_start(0.2).with_effects([MultiplyVolume(0.35)])
                    audio_clip = CompositeAudioClip([audio_clip, echo_clip])
            ...
            if scene["voice"]:
                ...
                self._add_subtitle_to_image(img_file, scene["voice"], temp_img_with_text, emoji, scene["character"], scene["is_punchline"])
        ...
        self._create_thumbnail(video_title)
        ...
        print("Generation Complete! Engine v9 finished.")

if __name__ == "__main__":
    import sys
    script = sys.argv[1] if len(sys.argv) > 1 else "script.md"
    output = sys.argv[2] if len(sys.argv) > 2 else "bragging_dog_v9.mp4"
    engine = JokeVideoEngineV9(script, output)
    engine.generate()