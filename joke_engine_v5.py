...
class JokeVideoEngine:
    def __init__(self, script_path, output_path="joke_video_v5.mp4", assets_dir="assets"):
        ...
        for i, scene in enumerate(scenes):
            ...
            if scene.get("is_punchline", False):
                ...
                if not os.path.exists(laugh_file):
                    try:
                        # Fallback simple laugh TTS
                        asyncio.run(self._generate_voice("哈哈哈哈！", laugh_file, "default"))
                    except:
                        pass
...
if __name__ == "__main__":
    ...
    output = sys.argv[2] if len(sys.argv) > 2 else "bragging_dog_v5.mp4"
